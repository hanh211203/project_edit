<?php
	...
	if(isset($_GET['id'])){
		$id= $_GET['id'];
		$sql= $mysqli->query("SELECT * FROM product WHERE id=$id"); //lay ra thong tin item
		$cat= $mysqli->query("SELECT * FROM product WHERE id=$id"); //lay ra id cat cua item
		$cat = $cat->fetch_object();
		$cat_id = $cat->category_id;
		$release = $mysqli->query("SELECT * FROM product WHERE cat_id=$cat_id AND id <> '$id' ORDER BY RAND() LIMIT 0,4"); //lay ra random nhung item co cung id cat
	}
	// lay URL
